...
function render() {
    ...
    setCircle(vertices[0], vertices[1], vertices[2], vertices[3]);

    gl.uniform1f(uX2, x2);                                              // ~ ~ ~
    gl.uniform1f(uY2, y2);
    gl.uniform1i(drawStatus, 2);
    setCircle(vertices[0], vertices[1], vertices[2], vertices[3]);   // trạng thái hình 2 = 2(int) => kiểm tra điều kiện ở vertex-shader

    gl.uniform1f(uX3, x3);                                              // ~ ~ ~
    gl.uniform1f(uY3, y3);
    gl.uniform1i(drawStatus, 3);
    setCircle(vertices[0], vertices[1], vertices[2], vertices[3]);   // trạng thái hình 3 = 3(int) => kiểm tra điều kiện ở vertex-shader
...
function setCircle(v1, v2, v3, v4) {
    var vertices = [v1, v2, v3, v4];                                    // tạo đỉnh
    spRender(vertices);                                                 // render
    gl.drawArrays(gl.TRIANGLE_FAN, 0, vertices.length);                 // vẽ hình
}
